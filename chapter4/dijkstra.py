...
def dijkstra(wg: WeightedGraph[V], root: V) -> Tuple[List[Optional[float]], Dict[int, WeightedEdge]]:
    first: int = wg.index_of(root) # 開始インデックスを求める
    ...
    while not pq.empty:
        u: int = pq.pop().vertex # 次の最近接点を探す
        dist_u: float = distances[u]
        for we in wg.edges_for_index(u): # 現在の節点からの全辺と節点を調べる
            dist_v: float = distances[we.v] # この節点への以前の距離
            if dist_v is None or dist_v > we.weight + dist_u: # 初訪問もしくは前回訪問時よりコストが低い場合
                distances[we.v] = we.weight + dist_u # 現在の節点への距離を更新
                path_dict[we.v] = we # この節点への最短経路の辺を更新
                pq.push(DijkstraNode(we.v, we.weight + dist_u)) # 後で調べる
    
    return distances, path_dict

# ダイクストラの結果を簡単に得るためのヘルパー関数
