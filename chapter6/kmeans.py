...
class KMeans(Generic[Point]):
    ...
    def _generate_centroids(self) -> None:
        for cluster in self._clusters:
            if len(cluster.points) == 0: # 点がないと同じ重心
                continue
            means: List[float] = []
    ...
    def run(self, max_iterations: int = 100) -> List[KMeans.Cluster]:
        for iteration in range(max_iterations):
            for cluster in self._clusters: # 全クラスタをクリア
                cluster.points.clear()
            self._assign_clusters()
            old_centroids: List[DataPoint] = deepcopy(self._centroids) # 重心を記録
            self._generate_centroids() # 新たな重心を計算
            if old_centroids == self._centroids: # 重心が収束したか
